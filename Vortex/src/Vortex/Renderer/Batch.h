...
namespace Vortex
{
	...
	public:
		...
		Batch(const Ref<Shader>& shader,
			  int maxUnitCount=10000)
		...
		void Flush(const glm::mat4& VP)
		{
			...
			int BUCnt = m_FreeVertexBufferBaseIndex / BU::GetVertexCount();
			Renderer::DrawIndexedTriangles(m_VertexArray, m_Shader, DrawTriangleConfig(m_FreeVertexBufferBaseIndex, BUCnt * BU::GetIndexCount()));
